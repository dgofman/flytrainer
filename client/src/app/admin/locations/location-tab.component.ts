...
import { LocationTabBaseDirective, LocationTabBaseModule } from '../location.component';
...
export class LocationTabComponent extends LocationTabBaseDirective implements OnInit {
    ...
    ngOnInit(): void {
       if (this.location && this.location.id) {
            /*this.loading(true);
            this.subs.add(this.adminService.getLocation(this.location.id).subscribe(result => {
                this.loading(false);
                this.selectedBean = Object.assign(this.defaultBean, result);
                Object.assign(this.location, this.selectedBean);
    ...
    onSubmit() {
        const model = new CommonModel(this.formGroup.value),
            location = model.location;
        this.loading(true);
        if (location && !location.id) {
            location.description = location.type + ' Location ' + location.type;
            location.isPrimary = 1;
        }
        /*if (location.id) {
            this.subs.add(this.adminService.updateLocation(location).subscribe(result => {
                this.loading(false);
            ...
            this.subs.add(this.adminService.createLocation(location).subscribe(result => {
                this.loading(false);
    ...
    get defaultBean() {
       return { type: AppUtils.getKey(AddressType, 'CFR'), notes: new Note(), location: new Address({type: AppUtils.getKey(AddressType, 'Business'), state: this.environment.homeState, country: this.environment.homeCountry}) };
    }
}

@NgModule({
    imports: [CommonModule, AdminSharedModule, LocationTabBaseModule],
