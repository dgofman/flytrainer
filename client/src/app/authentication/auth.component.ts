...
export class AuthComponent implements AfterViewInit, OnDestroy {
    ...
    constructor(private router: Router, private route: ActivatedRoute, private http: HttpClient, private appService: AuthService) {
        ...
        this.subs.add(http.get('/timezone/' + environment.timezone).subscribe(value => {
    ...
    ngAfterViewInit(): void {
        if (this.path === '/login') {
            ...
            if (Object.keys(params._value).length) {
                ...
                this.subs.add(this.http.post(this.path, Object.assign({ cid: this.environment.clientId, cip: this.worldtime.client_ip }, params._value)).subscribe(_ => {
    ...
    getMetars() {
        this.subs.add(this.http.get('/dataserver_current/httpparam?format=csv&dataSource=metars&requestType=retrieve&mostRecent=false&hoursBeforeNow=' +
    ...
    onSubmit(form: any) {
        ...
        if (form.valid) {
            ...
            AppUtils.loading(true);
            this.subs.add(this.http.post(this.path, Object.assign({ cid: this.environment.clientId, cip: this.worldtime.client_ip }, params._value, data)).subscribe((json: any) => {
                AppUtils.loading(false);
                switch (this.path) {
                    case '/login':
                        if (json.token) {
